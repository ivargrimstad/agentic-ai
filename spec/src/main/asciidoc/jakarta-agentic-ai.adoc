...
interceptors, and scoping. Agents can use standard CDI scopes (`@RequestScoped`,
`@ApplicationScoped`, etc.) or the custom `@WorkflowScoped` for agent-specific lifecycle management.
...
Agents support all standard CDI scopes:

- `@RequestScoped`: For agents that operate within a single request
- `@ConversationScoped`: For agents that maintain state across multiple interactions
- `@SessionScoped`: For user-specific agents
- `@ApplicationScoped`: For long-running monitoring agents
- `@WorkflowScoped`: Custom scope for agent workflow lifecycle (default)

If no scope is specified, `@WorkflowScoped` is assumed.
...
- **Thread Safety**: Agents using `@RequestScoped` or `@WorkflowScoped` typically execute within a single thread
...
Agents can use any CDI scope annotation in combination with `@Agent`:

[source,java]
----
// Request-scoped agent (for single-request workflows)
@Agent
@RequestScoped
public class SingleRequestAgent {
}

// Application-scoped agent (for long-running monitoring)
@Agent
@ApplicationScoped
public class MonitoringAgent {
}

// Workflow-scoped agent (default, for workflow-lifetime)
@Agent
@WorkflowScoped
...
- **Scope Activation**: For `@WorkflowScoped` beans, the scope is activated

==== Trigger Invocation

Triggers can be invoked by:

- **External Events**: System events, monitoring alerts, scheduled tasks
- **REST Endpoints**: HTTP requests to Jakarta REST resources
- **Messaging**: Jakarta Messaging (JMS) message reception
- **CDI Events**: Observing CDI events
- **Manual Invocation**: Direct method calls from application code
...
Decision methods support two return patterns:

**Boolean Pattern**:
- `true`: Proceed to action phase
- `false`: Terminate workflow (outcome phase skipped)

**Object Pattern**:
- Non-null object: Proceed to action phase; object available for injection
...
- **Side Effects**: Actions typically have side effects (API calls, database updates, etc.)
- **Return Value**: Return values are stored in workflow context for outcome phase
...
- **Finalization**: Produces the final workflow result
- **Action Dependent**: Invoked after action phase completes
- **Workflow Completion**: Marks the end of successful workflow execution
- **Scope Cleanup**: For `@WorkflowScoped` beans, scope may be deactivated after outcome
...
- **Any Phase**: Invoked when exceptions occur in any workflow phase
- **Optional**: Agents are not required to define exception handlers
...
1. The runtime searches for `@HandleException` methods
2. Methods accepting the thrown exception type (or supertype) are candidates
3. The most specific matching handler is invoked
4. If no handler matches, the exception propagates to the container
...
public class ExceptionHandlingExamples {
    ...
    @HandleException
    public void handleError(Exception e, WorkflowContext context) {
        Object triggerEvent = context.getTriggerEvent();
        logger.error("Failed processing " + triggerEvent, e);
    }
...
The `WorkflowContext` class represents the execution context for an agent workflow. It stores
...
A conversational agent that handles customer support inquiries:
...
1. **Different Scopes**: `@WorkflowScoped`, `@ApplicationScoped`, `@ConversationScoped`
